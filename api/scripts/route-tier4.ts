...
import { routeTier4Case } from '../src/lib/tier4-routing'
...
async function main() {
  const caseId = parseCaseId() || (await getLatestTierCase(4))

  if (!caseId) {
    console.log('No Tier 4 case found. Provide a case id via --caseId <id>.')
    return
  }
...
main()
  .catch(error => {
    console.error('Tier 4 routing script failed:', error)
    process.exitCode = 1
  })
  .finally(async () => {
