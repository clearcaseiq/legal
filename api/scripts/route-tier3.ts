...
import { routeTier3Case } from '../src/lib/tier3-routing'
...
async function main() {
  const caseId = parseCaseId() || (await getLatestTierCase(3))

  if (!caseId) {
    console.log('No Tier 3 case found. Provide a case id via --caseId <id>.')
    return
  }
...
main()
  .catch(error => {
    console.error('Tier 3 routing script failed:', error)
    process.exitCode = 1
  })
  .finally(async () => {
