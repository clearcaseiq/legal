...
// Cities, keywords and the excluded-domain blocklist are shared with the Google
// Places discovery pipeline; see src/lib/ca-pi-discovery-targets.ts.
const CA_CITIES = CA_DISCOVERY_CITIES
const PRACTICE_QUERIES = CA_DISCOVERY_KEYWORDS
...
function isExcludedDomain(domain: string) {
  return isExcludedDiscoveryDomain(domain)
}
