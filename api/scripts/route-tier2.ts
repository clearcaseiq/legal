...
import { routeTier2Case } from '../src/lib/tier2-routing'
...
async function main() {
  const caseId = parseCaseId() || (await getLatestTierCase(2))

  if (!caseId) {
    console.log('No Tier 2 case found. Provide a case id via --caseId <id>.')
    return
  }
...
main()
  .catch(error => {
    console.error('Tier 2 routing script failed:', error)
    process.exitCode = 1
  })
  .finally(async () => {
