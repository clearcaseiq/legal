 ...
 *
 * Goals:
 * - Fast assignment to qualified firms at a predictable price
...
const TIER_2_SUBSCRIPTION_TIMEOUT_SECONDS = 45
const TIER_2_FIXED_TIMEOUT_SECONDS = 90
const TIER_2_AUCTION_TIMEOUT_SECONDS = 120
...
const ANTI_MONOPOLY_WIN_SHARE_THRESHOLD = 0.35
const ANTI_MONOPOLY_MULTIPLIER = 0.85
const NEW_FIRM_MULTIPLIER = 1.05

const MS_PER_DAY = 24 * 60 * 60 * 1000

// Supported states (same as Tier 1)
const SUPPORTED_STATES = ['CA', 'NY', 'TX', 'FL', 'IL', 'PA', 'OH', 'GA', 'NC', 'MI']

type Jurisdiction = { state: string; counties?: string[] }

export interface Tier2FirmForRouting {
  id: string
  active: boolean
  ...
  subscription: {
    active: boolean
    tier2AllotmentRemaining: number
  }
  budgetRules: {
  ...
  tier2Offers30d: number
  tier2Wins7d: number
  attorney: any
}

export interface Tier2CaseData {
  id: string
  claimType: string
  venueState: string
...
export interface Tier2RoutingResult {
  routed: boolean
  routedToFirmId?: string
  introductionId?: string
  method?: 'subscription' | 'fixed_price' | 'auction'
  price?: number
  attempts?: {
    subscription: number
    fixedPrice: number
...
/**
 * Check if a case qualifies as Tier 2
 */
export function isTier2Case(
  claimType: string,
  facts: CaseFacts,
  estimatedValue: number,
  venueState?: string
  ...
  // Tier 2 typically has settlement value $25k-$100k
  if (estimatedValue < 25000 || estimatedValue > 100000) {
    return { isTier2: false, reason: `Estimated value $${estimatedValue} outside Tier 2 range ($25k-$100k)` }
  }

  // Check injury severity (Tier 2 typically 2-3, but can be lower)
  const maxSeverity = Math.max(
    0,
    ...(facts.injuries?.map(i => i.severity || 0) || []),
    facts.damages?.med_charges ? (facts.damages.med_charges > 25000 ? 2 : 1) : 0
  )
  const injurySeverityScore = maxSeverity

  // Check for surgery (Tier 2 can have minor surgery)
  const hasSurgery = facts.treatment?.some(t => t.surgery === true) || false

  // Check med_paid (Tier 2 typically $10k-$50k)
  const medPaid = facts.damages?.med_paid || 0
  if (medPaid > 50000) {
    return { isTier2: false, reason: `Medical paid $${medPaid} > $50k (likely Tier 3+)` }
  }

  // Check for catastrophic flags
  const hasPermanentDisability = facts.injuries?.some(i => i.permanent === true) || false
  const hasHighBills = (facts.damages?.med_charges || 0) >= 100000
  const hasCatastrophicFlags = hasPermanentDisability || hasHighBills

  if (hasCatastrophicFlags) {
    return { isTier2: false, reason: 'Case has catastrophic flags (likely Tier 3+)' }
  }

  // Check venue
  if (venueState && !SUPPORTED_STATES.includes(venueState.toUpperCase())) {
    return { isTier2: false, reason: `State ${venueState} not supported` }
  }
...
function estimatedValueBand(estimatedValue: number): string {
  if (estimatedValue < 25000) return '<$25k'
  if (estimatedValue <= 75000) return '$25k-$75k'
  if (estimatedValue <= 100000) return '$75k-$100k'
  if (estimatedValue <= 250000) return '$100k-$250k'
  return '>$250k'
}
...
function meetsCommercialEligibility(firm: Tier2FirmForRouting, price: number): boolean {
  const hasSubscription = firm.subscription.active && firm.subscription.tier2AllotmentRemaining > 0
...
type FirmStats = {
  ...
  weeklyCount: number
  monthlyCount: number
  tier2OfferCount30d: number
  tier2AcceptedCount30d: number
  tier2AcceptedTimeSeconds: number
  tier2AcceptedTimeCount: number
  tier2AcceptedCount7d: number
}
...
function buildStatsMap(
  ...
  todayStart: Date,
  sevenDaysAgo: Date
): { statsByAttorney: Map<string, FirmStats>; totalTier2Accepted7d: number } {
  const statsByAttorney = new Map<string, FirmStats>()
  let totalTier2Accepted7d = 0
  ...
  for (const intro of introductions) {
    const stat = statsByAttorney.get(intro.attorneyId) || {
      ...
      weeklyCount: 0,
      monthlyCount: 0,
      tier2OfferCount30d: 0,
      tier2AcceptedCount30d: 0,
      tier2AcceptedTimeSeconds: 0,
      tier2AcceptedTimeCount: 0,
      tier2AcceptedCount7d: 0
    }
    ...
    const isTier2 = intro.assessment?.caseTier?.tierNumber === 2
    if (isTier2) {
      stat.tier2OfferCount30d += 1
      if (intro.status === 'ACCEPTED') {
        stat.tier2AcceptedCount30d += 1
        if (intro.respondedAt) {
          const seconds = Math.max(1, (intro.respondedAt.getTime() - intro.requestedAt.getTime()) / 1000)
          stat.tier2AcceptedTimeSeconds += seconds
          stat.tier2AcceptedTimeCount += 1
        }
        if (intro.requestedAt >= sevenDaysAgo) {
          stat.tier2AcceptedCount7d += 1
          totalTier2Accepted7d += 1
        }
      }
    }
  ...
  return { statsByAttorney, totalTier2Accepted7d }
}

/**
 * STEP 0: Build Eligible Firm Pool for Tier 2
 */
export async function buildEligibleFirmPool(
  caseData: Tier2CaseData,
  price: number
): Promise<{
  eligible: Tier2FirmForRouting[]
  ineligible: Array<{ firmId: string; reason: string }>
}> {
  const eligible: Tier2FirmForRouting[] = []
  ...
  const { statsByAttorney, totalTier2Accepted7d } = buildStatsMap(
    recentIntroductions,
    todayStart,
    sevenDaysAgo
  )

  const firmCandidates: Tier2FirmForRouting[] = []
  ...
  for (const attorney of attorneys) {
    ...
    const tierEnabled =
      ...
      (subscriptionTier && (subscriptionTier.toLowerCase().includes('tier2') || subscriptionTier.toLowerCase() === 'premium')) ||
    ...
    if (!tierEnabled) {
      ineligible.push({ firmId: attorney.id, reason: 'Tier 2 not enabled' })
      continue
    }
    ...
    const acceptanceRate30d = stats.tier2OfferCount30d > 0
      ? stats.tier2AcceptedCount30d / stats.tier2OfferCount30d
      : acceptanceRateFallback

    let avgTimeToAcceptSeconds30d = DEFAULT_AVG_ACCEPT_SECONDS
    if (stats.tier2AcceptedTimeCount > 0) {
      avgTimeToAcceptSeconds30d = stats.tier2AcceptedTimeSeconds / stats.tier2AcceptedTimeCount
      ...
      avgTimeToAcceptSeconds30d = 30 + (1 - responseScore) * 120
    } else if (attorney.responseTimeHours > 0) {
      avgTimeToAcceptSeconds30d = Math.min(attorney.responseTimeHours * 3600, 900)
    }
    ...
    const firm: Tier2FirmForRouting = {
      id: attorney.id,
      ...
      responseSLASeconds: Math.max(30, Math.floor(avgTimeToAcceptSeconds30d)),
      acceptanceRate30d: clamp01(acceptanceRate30d),
      avgTimeToAcceptSeconds30d,
      qualityScore30d: clamp01(qualityScore30d),
      subscription: {
        active: subscriptionActive,
        tier2AllotmentRemaining: subscriptionRemaining
      },
      budgetRules: {
      ...
      tier2Offers30d: stats.tier2OfferCount30d,
      tier2Wins7d: stats.tier2AcceptedCount7d,
      attorney
    }
  ...
  let filtered = { eligible: [] as Tier2FirmForRouting[], ineligible: [] as Array<{ firmId: string; reason: string }> }
  ...
  const adjustedEligible = applyFairnessScores(filtered.eligible, totalTier2Accepted7d)
...
function filterEligibleFirms(
  firms: Tier2FirmForRouting[],
  caseData: Tier2CaseData,
  price: number,
  options?: EligibilityOptions
): { eligible: Tier2FirmForRouting[]; ineligible: Array<{ firmId: string; reason: string }> } {
  const eligible: Tier2FirmForRouting[] = []
  const ineligible: Array<{ firmId: string; reason: string }> = []
  ...
  for (const firm of firms) {
    ...
    if (!firm.tierEnabled) {
      ineligible.push({ firmId: firm.id, reason: 'Tier 2 not enabled' })
      continue
    }
...
function applyFairnessScores(firms: Tier2FirmForRouting[], totalTier2Accepted7d: number): Tier2FirmForRouting[] {
  ...
  return firms.map(firm => {
    const baseScore =
      0.35 * normalize(firm.acceptanceRate30d, minAcceptance, maxAcceptance) +
      0.25 * normalize(1 / Math.max(firm.avgTimeToAcceptSeconds30d, 1), minInverseTime, maxInverseTime) +
      0.25 * normalize(firm.qualityScore30d, minQuality, maxQuality) +
      0.15 * normalize(firm.capacity.openSlots, minOpenSlots, maxOpenSlots)

    let score = baseScore

    if (totalTier2Accepted7d > 0) {
      const share = firm.tier2Wins7d / totalTier2Accepted7d
      if (share > ANTI_MONOPOLY_WIN_SHARE_THRESHOLD) {
        score *= ANTI_MONOPOLY_MULTIPLIER
      }
    }

    if (firm.tier2Offers30d <= NEW_FIRM_MAX_OFFERS_30D) {
      score *= NEW_FIRM_MULTIPLIER
    }
...
function rankFirms(firms: Tier2FirmForRouting[]): Array<{ firm: Tier2FirmForRouting; score: number }> {
  if (firms.length === 0) return []

  return firms
    .map(firm => ({
      firm,
      score: (firm as Tier2FirmForRouting & { score?: number }).score || 0
    }))
...
async function sendOfferToFirm(
  ...
  // Send SMS so attorney can Accept/Decline via text
  await sendCaseOfferSms(firmId, intro.id, message.slice(0, 100), Math.ceil(TIER_2_FIXED_TIMEOUT_SECONDS / 60))
...
async function decrementSubscriptionOnAcceptance(firm: Tier2FirmForRouting): Promise<void> {
  const remaining = Math.max((firm.subscription.tier2AllotmentRemaining || 0) - 1, 0)
...
async function sendExclusiveOfferAndWait(
  ...
  const message = buildOfferMessage(caseData, price, timeoutSeconds, 'subscription')
  const introductionId = await sendOfferToFirm(caseData.id, firm.id, 'subscription', message)
...
async function sendFixedOfferToGroupFirstAccept(
  caseData: Tier2CaseData,
  firms: Tier2FirmForRouting[],
  price: number,
  timeoutSeconds: number
): Promise<{ firm: Tier2FirmForRouting; introductionId: string } | null> {
  if (firms.length === 0) return null

  const introductions: Array<{ firm: Tier2FirmForRouting; introductionId: string }> = []
  const message = buildOfferMessage(caseData, price, timeoutSeconds, 'fixed_price')

  for (const firm of firms) {
    const introId = await sendOfferToFirm(caseData.id, firm.id, 'fixed_price', message)
...
type AuctionResult = { firm: Tier2FirmForRouting; bid: number; introductionId: string }

async function runMiniAuction(
  caseData: Tier2CaseData,
  firms: Tier2FirmForRouting[],
  floorPrice: number,
  timeoutSeconds: number
  ...
  // NOTE: Full bidding flow is not yet implemented. We treat acceptance as floor bid.
...
async function markCaseHold(caseId: string): Promise<void> {
  ...
  logger.info('Case marked as Tier 2 hold', { caseId })
}

/**
 * MAIN TIER 2 ROUTING FUNCTION
 */
export async function routeTier2Case(caseId: string): Promise<Tier2RoutingResult> {
  try {
    ...
    if (!assessment.caseTier || assessment.caseTier.tierNumber !== 2) {
      return { routed: false, error: 'Case is not Tier 2' }
    }
    ...
    const tier2Check = isTier2Case(assessment.claimType, facts, estimatedValue, assessment.venueState)

    if (!tier2Check.isTier2) {
      return { routed: false, error: `Case does not qualify as Tier 2: ${tier2Check.reason}` }
    }
    ...
    const caseData: Tier2CaseData = {
      ...
      facts,
      tierNumber: 2,
      severityLevel: tier2Check.data!.injurySeverityScore,
      liabilityScore,
      docsAvailable,
    ...
    logger.info('Tier 2 eligible firms', {
      caseId,
    ...
    const subscriptionRanked = ranked.filter(entry => entry.firm.subscription.active && entry.firm.subscription.tier2AllotmentRemaining > 0)
    const maxAttempts = Math.min(MAX_SUBSCRIPTION_ATTEMPTS, subscriptionRanked.length)

    for (let i = 0; i < maxAttempts; i++) {
      const firm = subscriptionRanked[i].firm
      attemptedFirmIds.add(firm.id)

      const offerResult = await sendExclusiveOfferAndWait(caseData, firm, price, TIER_2_SUBSCRIPTION_TIMEOUT_SECONDS)

      if (offerResult.accepted) {
        await decrementSubscriptionOnAcceptance(firm)

        logger.info('Tier 2 subscription offer accepted', {
          caseId: caseData.id,
          firmId: firm.id,
      ...
      logger.info('Tier 2 subscription offer declined/timeout', {
        caseId: caseData.id,
        firmId: firm.id,
        introductionId: offerResult.introductionId
      })
    ...
    // Phase B: Fixed price top K, first accept wins
    const fixedRanked = ranked.filter(entry =>
      entry.firm.budgetRules.fixedEnabled &&
      entry.firm.budgetRules.maxPriceByTier >= price &&
      !attemptedFirmIds.has(entry.firm.id)
    )

    const fixedCandidates = fixedRanked.slice(0, FIXED_GROUP_K).map(entry => entry.firm)
    const fixedWinner = await sendFixedOfferToGroupFirstAccept(caseData, fixedCandidates, price, TIER_2_FIXED_TIMEOUT_SECONDS)

    if (fixedWinner) {
      logger.info('Tier 2 fixed-price offer accepted', {
        caseId: caseData.id,
        firmId: fixedWinner.firm.id,
        introductionId: fixedWinner.introductionId,
        price
      })

      return {
        routed: true,
        routedToFirmId: fixedWinner.firm.id,
        introductionId: fixedWinner.introductionId,
        method: 'fixed_price',
        price,
        attempts: {
          subscription: maxAttempts,
          fixedPrice: fixedCandidates.length,
          auction: 0
        }
      }
    }

    // Phase C: Mini-auction fallback (optional)
    const auctionRanked = ranked.filter(entry =>
      entry.firm.budgetRules.auctionEnabled &&
      !attemptedFirmIds.has(entry.firm.id)
    )

    const auctionCandidates = auctionRanked.slice(0, AUCTION_GROUP_M).map(entry => entry.firm)
    const auctionWinner = await runMiniAuction(caseData, auctionCandidates, price, TIER_2_AUCTION_TIMEOUT_SECONDS)

    if (auctionWinner) {
      logger.info('Tier 2 auction accepted', {
        caseId: caseData.id,
    ...
    logger.error('Error in Tier 2 routing', { error, caseId })
    return {
      routed: false,
