...
const INSURANCE_COMMON: BankQuestion[] = [
  ...
  { id: 'ins_offer', section: 'Insurance', text: 'Has any insurer made you a settlement offer, and if so, for how much?', whyAsked: 'An existing offer anchors negotiation and shows how the carrier values the claim.', valueImpact: 'medium' },
  { id: 'ins_health_liens', section: 'Insurance', text: 'Do you have health insurance, Medicare or Medi-Cal, and has any of it paid for treatment from this incident?', whyAsked: 'Whoever paid for treatment may hold a lien on the settlement, which changes what the client takes home.', valueImpact: 'medium' },
  { id: 'ins_um', section: 'Insurance', text: 'Do you carry uninsured/underinsured motorist (UM/UIM) coverage on your own auto policy?', whyAsked: 'UM/UIM is the entire recovery when the at-fault driver is uninsured, and it backstops a low defendant limit.', valueImpact: 'high', gate: (g) => g.has('first_party_coverage') || g.has('defendant_policy_limits'), gapKeys: ['first_party_coverage'] },
  { id: 'ins_medpay', section: 'Insurance', text: 'Does your own auto or health policy include MedPay or PIP (no-fault medical) coverage?', whyAsked: 'MedPay/PIP pays treatment regardless of fault, so it keeps the client treating while liability is contested.', valueImpact: 'high', gate: onlyIfGap('first_party_coverage'), gapKeys: ['first_party_coverage'] },
  { id: 'ins_own_declarations', section: 'Insurance', text: 'Can you send us the declarations page for your own auto policy?', whyAsked: 'The declarations page is the only reliable proof of what first-party coverage is actually available.', valueImpact: 'medium', gate: onlyIfGap('first_party_coverage'), gapKeys: ['first_party_coverage'] },
]
...
const BANKS: Record<string, BankQuestion[]> = {
  auto: [
    ...
    { id: 'auto_own_carrier', section: 'Insurance', text: 'Who is your own auto insurance company?', whyAsked: 'Needed to open any UM/UIM, MedPay or PIP claim on the client’s own policy.', valueImpact: 'medium' },
  ],
  dog_bite: [
