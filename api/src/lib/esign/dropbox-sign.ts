...
/**
 * Who Dropbox Sign names as the sender is not something this adapter can set.
 *
 * Claimants see the API key holder's own profile — "Sridhar Reddy" and their
 * personal mailbox — in the From line, the "X has requested a signature" line
 * and the audit trail. This file used to send `custom_requester_name` and
 * `custom_requester_email_address` to fix that. Neither exists: they appear
 * nowhere in the Dropbox Sign API, `requester_email_address` is a field on the
 * *response*, and the send endpoint ignores anything it does not recognise. So
 * the code claimed to brand the mail while changing nothing, and the retry it
 * carried for a rejection that could never arrive made it look deliberate.
 *
 * The sender identity is a property of the account the API key belongs to, and
 * the fixes are all outside this codebase:
 *
 *   - rename that account's profile to the product and give it a generic
 *     mailbox (the approach Dropbox Sign itself recommends), or
 *   - set a From address under account-level Branding, on a Standard or
 *     Premium API plan, or
 *   - move to Embedded Requesting or OAuth, where `requester_email_address` on
 *     an unclaimed draft does select a sender — and only one already on the
 *     team.
 */
