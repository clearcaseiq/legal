/**
 * Client invoicing for a single case: invoices, payments, and recurring
 * invoices.
 *
 * Extracted from AttorneyDashboardWorkstreamBilling so the same markup serves
 * both the legacy dashboard workstream and the case workspace's Billing tab.
 * This deliberately excludes the Stripe *platform* billing blocks (subscription
 * tiers, lead credits, Connect onboarding) — those are account-level concerns
 * that belong on the attorney's billing page, not on a case.
 */
type Props = {
  invoiceForm: any
  setInvoiceForm: any
  ...
  invoiceItems: any[]
  handleDownloadInvoicePdf: (invoiceId: string) => void
  handleDownloadInvoiceDocx: (invoiceId: string) => void
  handlePayInvoiceWithStripe: (invoiceId: string) => void
  paymentForm: any
  setPaymentForm: any
  handleAddPayment: any
  paymentItems: any[]
  handleDownloadPaymentReceipt: (paymentId: string) => void
  recurringInvoiceForm: any
  setRecurringInvoiceForm: any
  handleProcessRecurringInvoices: () => void
  handleAddRecurringInvoice: () => void
  recurringInvoices: any[]
}

export default function CaseInvoicingSections({
  invoiceForm,
  setInvoiceForm,
  handleAddInvoice,
  invoiceItems,
