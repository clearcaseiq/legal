  ...
  return (
    <div className="mt-3 space-y-3">
      {regions.map((regionId) => {
        ...
        const symptoms = [...config.symptoms, ...extraRedFlagSymptoms(regionId)]
                  ...
                  {SIDE_OPTIONS.map(({ value: sv, label }) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center justify-center gap-1.5 rounded-lg border px-2 py-1.5 text-sm font-medium transition-colors ${
                          selected
                            ? 'border-brand-500 bg-brand-50/70 text-brand-800 dark:border-brand-500/50 dark:bg-brand-500/10 dark:text-brand-200'
                            : 'border-slate-200 bg-white text-gray-800 hover:border-brand-300 dark:border-slate-700 dark:bg-slate-900/40 dark:text-slate-200'
                        }`}
                      >
                ...
                {symptoms.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      aria-pressed={selected}
                      onClick={() => patchRegion(regionId, { symptoms: toggleInArray(detail.symptoms, s.id) })}
                ...
                {config.findings.map((f) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      aria-pressed={selected}
                      onClick={() => patchRegion(regionId, { findings: toggleInArray(detail.findings, f.id) })}
                      className={chipClass(selected)}
                    >
                      <span className="min-w-0 flex-1 [overflow-wrap:anywhere] text-[13px] font-semibold leading-tight text-gray-800 dark:text-slate-200 sm:text-xs">
                        {f.label}
                ...
                {config.treatments.map((tItem) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      aria-pressed={selected}
                      onClick={() => patchRegion(regionId, { treatments: toggleInArray(detail.treatments, tItem.id) })}
                      className={chipClass(selected)}
                    >
                      <span className="min-w-0 flex-1 [overflow-wrap:anywhere] text-[13px] font-semibold leading-tight text-gray-800 dark:text-slate-200 sm:text-xs">
                        {tItem.label}
