...
const typeLabel = (t?: string | null) =>
  t === 'coach' ? 'AI Coach' : t === 'question' ? 'Plaintiff Questions' : t ? TASK_TYPE_LABEL[t] || claimLabel(t) : '—'

/** Small pill that flags a task auto-generated by the AI Case Coach. */
function AiTaskBadge() {
  return (
    <span
      className="inline-flex items-center gap-1 rounded bg-violet-50 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-violet-700 ring-1 ring-violet-200"
      title="Auto-generated by the AI Case Coach"
    >
...
/** Small pill that flags the task holding this case's Intelligent Questions. */
function QuestionTaskBadge() {
  return (
    <span
      className="inline-flex items-center gap-1 rounded bg-sky-50 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-sky-700 ring-1 ring-sky-200"
      title="Questions to work through with the plaintiff — open the task for the checklist"
    >
...
/** The AI/question origin badge for a task row (nothing for manual tasks). */
function TaskOriginBadge({ taskType }: { taskType?: string | null }) {
  if (taskType === 'coach') return <AiTaskBadge />
